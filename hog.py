...
def free_bacon(score):
    ...
    while power>=1:
        if turn%2==0:
            point=power%factor+point
            power=power//factor
        else:
            point=point-power%factor
            power=power//factor
        turn += 1
    return abs(point)+1
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    sum_two_player_score=player_score+opponent_score
    three_power=3**sum_two_player_score
    end=3**sum_two_player_score%10
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who==0:
            if abs(strategy0(score0,score1)-previous_score0) == 2 and feral_hogs:
                extra=3
            else:
                extra=0
            previous_score0=take_turn(strategy0(score0,score1), score1, dice)
            score0=score0+previous_score0+extra
        elif who==1:
            if abs(strategy1(score1,score0)-previous_score1) == 2 and feral_hogs:
                extra=3
            else:
                extra=0
            previous_score1=take_turn(strategy1(score1,score0), score0, dice)
            score1=score1+previous_score1+extra
        who=other(who)
        swap_test=is_swap(score0,score1)
        if swap_test:
...
def make_averaged(g, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def calc_average(*arg):
        turn=0
        total=0
        while turn<num_samples:
            total=total+g(*arg)
            turn+=1
...
"""Test Round"""
def test_01():
    for opp_score in range(0,99):
        for my_score in range(0,99):  
            if swap_strategy(my_score, opp_score, 100, 10)==0:
                if opp_score-my_score > 50:
                    print(opp_score,my_score,opp_score-my_score)
            
        
def try_free():
    for i in range(1,100):
        if free_bacon(i)>8.7:
            print(i,free_bacon(i))
